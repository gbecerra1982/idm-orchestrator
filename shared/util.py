...
def chat_complete(messages, functions, function_call='auto'):
    ...
    data = {
        ...
        "max_completion_tokens": int(AZURE_OPENAI_RESP_MAX_TOKENS)
    }
...
async def chat_completion(system_prompt, user_prompt, context, images=[]):
    """  Return assistant chat response based on user query. Assumes existing list of messages """

    # Fill user prompt with variables
    filled_prompt = fill_template(user_prompt, context)
    ...
    try:
        ...
        logging.error(f"[util] get_completion Error: {str(e)}")
        completion = get_message('ERROR_ANSWER')
        return completion

    if response_data["choices"][0]["finish_reason"] == "content_filter":
        completion = get_message('BLOCKED_ANSWER')
    else:
        completion = response_data["choices"][0]["message"]["content"]
    
    response_time =  round(time.time() - start_time,2)  
    logging.info(f"[util] finished generating bot answer. {response_time} seconds. {completion[:100]}.")
...
def get_aoai_config(model):
    ...
    elif model in ('o1', 'o3-mini'):
        deployment = os.environ.get("AZURE_OPENAI_CHATGPT_ANSWER_DEPLOYMENT")
