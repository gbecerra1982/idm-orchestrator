...
class RAG:
    ...
    def Retrieval(self, input: str, context: SKContext) -> str:
        ...
        try:
            ...
            # prepare body
            body = {
                "select": "title, content, url, filepath, chunk_id, document_level, language, process, document_type, country, metadata_storage_name, chunk_headers, relatedImages",
            ...
            if AZURE_SEARCH_APPROACH == TERM_SEARCH_APPROACH:
                ...
                for meta_field in meta_field_list:
                    if context.variables.get(meta_field):
                        meta_field_list_dict.append({"meta_name": meta_field, "meta_value": context.variables.get(meta_field)})
            ...
            if status_code >= 400:
                ...
                if response.json()['value']:
                        for doc in response.json()['value']:
                            search_results.append(doc['metadata_storage_name'] + ": "+ doc['content'].strip() + "\n" + f"relatedImages: {str(doc['relatedImages'])}" + "\n")    
